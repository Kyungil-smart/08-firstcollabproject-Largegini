...
{
    // 딕셔너리로 받기
    public Dictionary<EBlockType, BlockDataSO> BlockDic { get; private set; } = new Dictionary<EBlockType, BlockDataSO>();


    // CSVReader 스크립트를 활용해 CSV 파일 읽어서 BlockDataSO 타입에 저장하기
    public bool LoadCSVFile(string path)
    {
        ...
        foreach (var row in dataList)
        {
            ...
            newBlock.SetBlockData(row);

            // 중복 ID가 아니라면 MonsterDic 에 추가 (중복이면 가장 먼저 온 ID가 읽어짐)
            if (!BlockDic.ContainsKey(newBlock.Type)) BlockDic.Add(newBlock.Type, newBlock);
