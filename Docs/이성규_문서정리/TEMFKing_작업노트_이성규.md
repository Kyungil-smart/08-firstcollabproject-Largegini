...
### 스프라이트 아틀라스 패킹 (최적화)
- 공통 UI, 퍼즐, 스킬 아이콘, 몬스터 프레임(폴더 단위)을 용도별 아틀라스로 패킹하여 드로우콜 및 메모리 최적화.
- **제외 대상**: 타이틀, Game Over 등 1회성 대형 이미지 및 참조가 꼬일 수 있는 외부 에셋스토어 리소스.
- **주요 설정**: 원본 에셋 압축 `None`으로 변경 (이중 압축 방지), `Allow Rotation` 체크 해제 (UI/이펙트 회전 출력 방지).

### 빌드 환경 퍼즐 SFX 볼륨 조절 버그 수정
